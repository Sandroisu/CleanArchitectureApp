...
private fun isPolindrome(chars: CharArray, middlePoint: Int): String {
    var maxLengthString = ""
    var leftIndex = middlePoint
    for (i in middlePoint downTo 0) {
        if (chars[middlePoint] == chars[i]) {
            leftIndex = i
        } else {
            break
        }
    }
    var rightIndex = middlePoint
    for (i in middlePoint until chars.size) {
        if (chars[middlePoint] == chars[i]) {
            rightIndex = i
        } else {
            break
    ...
    for (i in leftIndex downTo 0) {
        val oppositeIndex = rightIndex + i
        if (oppositeIndex > chars.size - 1) {
            val subchars = chars.slice(i + 1..<chars.size).toCharArray()
            return String(subchars)
        }
        if (oppositeIndex == chars.size - 1) {
            val subchars = chars.slice(i..<chars.size).toCharArray()
            return String(subchars)
        }
        if (chars[i] == chars[oppositeIndex]) {
            val subchars = chars.slice(i..oppositeIndex).toCharArray()
            maxLengthString = String(subchars)
        } else {
            break
        }
    }
...
fun main() {
    fun longestPalindrome(s: String): String {
        ...
        for (i in chars.indices) {
            ...
            val newString = isPolinme(chars, i)
    ...
    fun isPolinme(chars: CharArray, middlePoint: Int): String {
        var maxLengthString = ""
        var leftIndex = middlePoint
        for (i in middlePoint downTo 0) {
            if (chars[middlePoint] == chars[i]) {
